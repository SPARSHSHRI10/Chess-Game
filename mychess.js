window.onload = function(){
    ...
    function check(){ //function called when a white piece is clicked
        if(myTurn){
            var n = Number(this.classList[1].slice(1));
            var target = values[n];

            var scopes = checkBlack(n,values) || []; //finds scope of the white piece being clicked and stores them in this array
            ...
            if(!moveable){ 
                if(scopes.length > 0){
                    moveable = true;
                    moveTarget = n;
                    moveScopes = scopes.join(",").split(","); //converts array into string using join then changes it into substring using split 
                }
                else {
                ...
                if(moveScopes.indexOf(String(n)) >= 0){
                    ...
                    var saveKing = false;
                    for(var z = 0; z < 64; z++){
                        checkArr[z] = values[z];  //checkArr being made which stores values array in it 
                    }
                    ...
                    for(var y = 0; y < 64; y++){
                        if("prnbkq".indexOf(checkArr[y]) >= 0){ //now in checkArr for all 64 squares check if any piece of comp(black) is present
                            var checkScp = checkWhite(y,checkArr) || []; // if yes then checkWhite is called to find all scopes of that black piece
                            for(var z = 0; z < checkScp.length; z++){
                                if(checkArr[checkScp[z]] === 'l'){ //if scope of any black piece has king of white("l") in its scope then it will give check
                                    if(!saveKing){
                                        alert('Save Your King'); //this makes saveking true hence castling can't be done by white(us) now
                                        saveKing = true;
                                    }
                                }
                            }
                    ...
                    if(!saveKing){  //if still not checked by opponent then castling can be done
                        ...
                        if(values[n] === "o" && n < 8){  //reviving white queen when white pawn reaches its last row(0-7)
                            values[n] = "w";
                        }
                        moveable = false;   
                        scopes = [];         //scopes made empty
                        myTurn = false;     //now my turn gone so its computers turn therefore myTurn set to false
                        setTimeout(chooseTurn,1000);  //setTimeout called
                    }
                }
                else {
                    moveScopes = [];
            ...
            for(var x = 0; x < scopes.length; x++){
                sqs[scopes[x]].style.background = "#f45";  //coloring scope as red to show possible movements
            //    alert(scopes)
            }
        }
    }
